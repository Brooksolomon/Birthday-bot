...
dotenv.config();
import { FireFunc } from "./botFirebase";
...
const sendBirthdayMessage = async (chatId: string, username: string) => {
...
const checkBirthdays = async () => {
  ...
  const birthdays: ChatData = await FireFunc.getAllBirthdays();
...
bot.command("addbirthday", (ctx) => {
    ...
    FireFunc.addBirthday(String(ctx.message.chat.id), username, date)
...
bot.command("upcomingbirthdays", async (ctx) => {
  if (!ctx.message || !ctx.message.text) {
    ...
    const upcomingBirthdays = await FireFunc.getUpcomingBirthdays(chatId, range as "1w" | "1m" | "3m");
...
bot.command("listbirthdays", async (ctx) => {
  if (!ctx.message || !ctx.message.text) {
    return ctx.reply("⚠️ Unable to process the message.");
}
    const chatId = String(ctx.message.chat.id);
    const birthdays =await  FireFunc.getBirthdays(chatId);
